...
echo ' 
<h4 class="text-center"> edit your topic</h4>';
...
if (is_file(GW_UPLOADPATH . $row['picture']) && filesize(GW_UPLOADPATH . $row['picture']) > 0) {
  ...
  echo '<form role="form" enctype="multipart/form-data" method="post" action="edittopic.php?category='.$category.'">';
	...
	echo '<input type="hidden" name="oldpicture" value="' .$row['picture']. '" />';
	  echo '<span id="helpBlock" class="help-block"> alter or add new picture... max '.$maxupload.' </span>';
  ...
  echo '</form>';
     ...
     echo '<hr />';
    echo'<fieldset>';
	echo'<strong> Delete the Topic?? </strong>';
	echo'<span id="helpBlock" class="help-block">select yes if you wish to delete this topic</span>';
