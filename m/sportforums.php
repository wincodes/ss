 ...
 $user_id =  $_SESSION['user_id'];
  ...
  $picture = $_FILES['picture']['name'];
      ...
      if ((($picture_type == 'image/gif') || ($picture_type == 'image/jpeg') || ($picture_type == 'image/pjpeg') || ($picture_type == 'image/png'))
        ...
        if ($_FILES['picture']['error'] == 0) {
          ...
          if (move_uploaded_file($_FILES['picture']['tmp_name'], $target)) {
            ...
            $query = "INSERT INTO topics (user_id, username, category, topic, text, date, picture) VALUES ($user_id,'$username', '$category1', '$topic', '$text',NOW(), '$picture')";
    ...
    if (!empty($topic) && !empty($text)) {
	       ...
	       // Write the data to the database
        $query = "INSERT INTO topics (user_id, username, category, topic, text, date) VALUES ($user_id,'$username', '$category1', '$topic', '$text',NOW())";
...
require_once('mobilesoccernetworks.php');
}else{
...
<?php            
echo '<P  class="text-center"><strong>'.$category.' Forum </strong></p>';
...
//check if topic is not set if it is not tell the user it is not set
 ...
 if (is_file(GW_UPLOADPATH . $row['picture']) && filesize(GW_UPLOADPATH . $row['picture']) > 0) {
      echo '<img src="' . GW_UPLOADPATH . $row['picture'] . '" class="img-responsive center-block" alt="image" />';
	  	...
	  	echo '<br />';	
	echo '<div id = topic1>';
	echo '<h4 class="text-primary text-center"> ' .$row['topic']. '</h4>'; 
    echo '</div>';
...
echo'<br />'; 
 ...
 //Get the comments or replies
  echo '<hr width="100%" size="0.1" align="center">';
  $query2 = "SELECT * FROM topics_reply WHERE topic_id = $topic ORDER BY reply_id ASC";
...
echo '<hr width="100%" size="0.1" align="center"></p>'; 
 }
...
if (isset($_SESSION['user_id']) && isset($_SESSION['username'])) {
  echo '<hr />';
  echo '<form method="post" action="sportforums.php?topic='.$topic.'&category='.$category.'" >';
    echo '<textarea class="form-control" name="text" rows="5" cols="20"> </textarea><br />';
  ...
  echo '<hr />';
  echo '<form role="form" method="post" action="sportforums.php?topic='.$topic.'&category='.$category.'" >
  ';
    echo '
...
$result = mysqli_query($dbc, $sql);
  ...
  while ($row = mysqli_fetch_array($result)) {
    echo '<div id = topic>';
	echo '<div id = topic1>';
	echo '<p class="text-center"><strong><a href="sportforums.php?topic='.$row['topic_id'].'&category='.$category.'&/'.$row['topic'].'">' .$row['topic']. '</a></strong></p>'; 
 	echo '</div>';
    echo '<p class="text-justify">' . substr($row['text'], 0, 100) . '......</p>';
	$query2 = "SELECT * FROM topics_reply WHERE topic_id = ".$row['topic_id']."";
	$result2 = mysqli_query($dbc, $query2);
	$numrows = mysqli_num_rows ($result2);
    echo '<small>Posted by: <a href="viewprofile.php?username='.$row['username'].'">'.$row['username'].'</a>, ' . substr($row['date'], 0, 10) . ', '.$numrows.' comments</small>';
 	echo '<hr width="100%" size="0.1" align="center">';
 echo '</div>';
  } 
...
require_once('../connectvars.php');
 ...
 $result = mysqli_query($dbc, $query);
  ...
  echo '<div id = topic1>';
    ...
    echo '</div>';
  echo '<p class="text-justify">' . substr($row['text'], 0, 200) . '...</p>';
  $query2 = "SELECT * FROM topics_reply WHERE topic_id = ".$row['topic_id']."";
  $result2 = mysqli_query($dbc, $query2);
  $numrows = mysqli_num_rows ($result2);
