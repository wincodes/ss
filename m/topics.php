...
<?php            
echo '<P  class="text-center"><strong>'.$category.' Topics </strong></p>';
...
$result = mysqli_query($dbc, $sql);
  ...
  while ($row = mysqli_fetch_array($result)) {
    echo '<div id = topic>';
	echo '<div id = topic1>';
	echo '<p class="text-center"><strong><a href="topics.php?topic='.$row['topic_id'].'&category='.$category.'&/'.$row['topic'].'">' .$row['topic']. '</a></strong></p>'; 
 	echo '</div>';
