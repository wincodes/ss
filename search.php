...
if(empty($_GET['search'])){echo '<h4> Please enter a valid word to search sport socia topics</h4>';
	exit();
  }
...
$result = mysqli_query($dbc, $sql);
  ...
  while ($row = mysqli_fetch_array($result)) {
   echo '<strong>';
	echo '<a href="sportforums.php?category='.$row['category'].'&topic='.$row['topic_id'].'">' .$row['topic']. '</a><br />'; 
 	echo '</strong>';
    echo '<p class="text-justify"> ' . substr($row['text'], 0, 150) . '...</p>';
    echo '<small>Posted by: '. $row['username'] . ', ' . substr($row['date'], 0, 10) . '</small>';
 	echo '<hr width="90%" size="2" align="center">';
   } 
