...
$result = mysqli_query($dbc, $sql);
  ...
  while ($row = mysqli_fetch_array($result)) {
echo '<strong><a href="sportforums.php?topic='.$row['topic_id'].'&category='.$category.'&/'.$row['topic'].'">' .$row['topic']. '</a></strong>'; 
...
//the category is not set display recent topics and other stuffs
echo'<div class="col-sm-2 col-md-2">';
  require_once('sportnetworks.php');
  require('ads.php');
echo'</div>';
